...
extension String {
    ...
    /**
     ...
     @param limitStr 例如 "1234567890"
     @return 当前字符串 为limitStr范围内的字符 返回true
     */
    func limitStringRange(_ limitStr: String) -> Bool {
        // 1.获取给定字符串的unicode，并取得除给定字符串内容外的所有unicode
        let cSet = NSCharacterSet.init(charactersIn: limitStr).inverted
        
        // 2.通过给定字符串内容外的unicode进行分割，并将分割后的字符数组拼接为一个完整字符串
        let newStr = self.components(separatedBy: cSet).joined()
        
        // 判断原字符串是否与新字符串相同 （相同则在范围内，不同则在范围外）
        return self == newStr
    }
}
