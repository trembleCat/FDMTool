...
extension LoginController {
    ...
    /**
     获取协议内容
     */
    func getTreatyContent() {
        HTTPProvider<UserApi<TreatyModel>>().request(.getUserTreatyContent(role: treatyType)) { (response) in
            
            if response.value != nil {
                self.treatyModel = response.value!
                self.clauseView.setTreatyModel(model: response.value!)
            }
        }
    }
     ...
     */
    func checkClause() {
        let isClause = UserDefaults.standard.bool(forKey: isAgreeClause)
        
        if !isClause {  // isClause == false 没有同意协议
            showClauseView()
        }
    }
     ...
     */
    func showClauseView() {
        self.clauseView.alpha = 0
        self.clauseView.clickUserProtocolBlock = { [weak self] in   // 用户协议
            self?.clickUserProtocol()
        }
        
        self.clauseView.clickProtocalActionBlock = { [weak self] in    // 隐私政策
            self?.protocalAction()
        }
    ...
    func setStaticGuidePage() {
        //判断当前版本是否第一次启动
        if UserDefaults.isFirstLaunchOfNewVersion() {   // 显示引导页
            ...
            guideView.closeHudBlock = { [weak self] in
                self?.showClauseView()
            }
 ...
 */
    ...
    /**
     点击用户协议
     */
    @objc func clickUserProtocol() {
//        self.pushPDFController(fileName: "userProtocol", titleName: "书香阅读用户服务协议")
        
        guard self.treatyModel != nil else {
            SXToast.showToastAction(message: "获取协议内容失败")
            self.getTreatyContent()
            return
        }
        
        let contenttData = self.treatyModel?.content2.data(using: .unicode)
        let contentHtmlStr = try? NSAttributedString.init(data: contenttData ?? Data(), options: [NSAttributedString.DocumentReadingOptionKey.documentType : NSAttributedString.DocumentType.html], documentAttributes: nil)
        
        let vc = TreatyController(attrStr: contentHtmlStr ?? NSAttributedString(), title: "书香阅读用户服务协议")
    ...
    /**
     点击服务条款
     */
    @objc func protocalAction(){
//        self.pushPDFController(fileName: "privacyProtocol1.1", titleName: "书香阅读隐私政策协议")
        ...
        let contenttData = self.treatyModel?.content1.data(using: .unicode)
        let contentHtmlStr = try? NSAttributedString.init(data: contenttData ?? Data(), options: [NSAttributedString.DocumentReadingOptionKey.documentType : NSAttributedString.DocumentType.html], documentAttributes: nil)
        
        let vc = TreatyController(attrStr: contentHtmlStr ?? NSAttributedString(), title: "书香阅读隐私政策协议")
        self.navigationController?.pushViewController(vc, animated: true)
    }
    ...
    /**
     进入PDF页面
     */
    func pushPDFController(fileName: String, titleName: String) {
        let documentFileURL = Bundle.main.url(forResource: fileName, withExtension: "pdf")!
        let document = PDFDocument(url: documentFileURL)!
        let readerController = PDFViewController.createNew(with: document,title: titleName,backButton:backBarBtnItem)
